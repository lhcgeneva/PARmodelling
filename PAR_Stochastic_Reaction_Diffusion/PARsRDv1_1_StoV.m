...
%
% This version also attempts to take into account local SA/V, by
% making on rates proportional to local curvature.
...
% conversion
konA_o = konA * Si / totalV; % Adjust for concentration to N conversion
konB_o = konB * Si / totalV; % Adjust for concentration to N conversion
% Get curvature for ellipsoid, this only works for reflecting boundary
% conditions for now:
t = 0:pi/(bins-1):pi;
a = 27*L/134.6*2;
b = 15*L/134.6*2;
...
%%% Initial conditions
if numDom == 2
    A(1:boundPos,1) = 1 * floor(konA_o * A0 / (konA_o*bins+koffA)); % Init distribution 
    B(boundPos+1:end,1) = 1 * floor(konB_o * B0 / (konB_o*bins+koffB));
elseif numDom == 4
    A(1:floor(boundPos/2),1) = 1 * floor(konA_o * A0 / (konA_o*bins+koffA)); % Init distribution 
    B(floor(boundPos/2)+1:2*floor(boundPos/2),1) = 1 * floor(konB_o * B0 / (konB_o*bins+koffB));
    A(2*floor(boundPos/2)+ 1:3*floor(boundPos/2),1) = 1 * floor(konA_o * A0 / (konA_o*bins+koffA)); % Init distribution 
    B(3*floor(boundPos/2)+1:end,1) = 1 * floor(konB_o * B0 / (konB_o*bins+koffB));
elseif numDom == 3
    A(floor(p * bins) + 1:floor((0.5+p)*bins), 1) = 1 * floor(konA_o * A0 / (konA_o*bins+koffA));
    B(1:floor(p * bins),1) = 1 * floor(konB_o * B0 / (konB_o*bins+koffB));
    B(floor((0.5+p)*bins)+1:end,1) = 1 * floor(konB * B0 / (konB_o*bins+koffB));
